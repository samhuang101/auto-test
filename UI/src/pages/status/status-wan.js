	...
	function set_ui_init(obj) {
		...
		for (i = 0; i < obj.Ipv4.length; ++i) {
			var conn_st = obj.Ipv4[i].ConnectStatus.toLowerCase();

			$('#ipv4conn_name'+(i+1)).text(obj.Ipv4[i].ConnectionName);
			$('#ipv4type'+(i+1)).text(obj.Ipv4[i].Type);
			$('#ipv4status'+(i+1)).text(eval("L.status_wan.lang_wan_st_"+conn_st));

			if (conn_st == "connected")
				$('#ipv4address'+(i+1)).text(obj.Ipv4[i].IpAddress);
			else
				$('#ipv4address'+(i+1)).text("");
		}

		for (i = 0; i < obj.Ipv6.length; ++i) {
			var conn_st = obj.Ipv6[i].ConnectStatus.toLowerCase();

			$('#ipv6conn_name'+(i+1)).text(obj.Ipv6[i].ConnectionName);
			$('#ipv6type'+(i+1)).text(obj.Ipv6[i].Type);
			$('#ipv6status'+(i+1)).text(eval("L.status_wan.lang_wan_st_"+conn_st));

			if (conn_st == "connected")
				$('#ipv6address'+(i+1)).text(obj.Ipv6[i].IpAddress);
			else
				$('#ipv6address'+(i+1)).text("");
		}
	}
